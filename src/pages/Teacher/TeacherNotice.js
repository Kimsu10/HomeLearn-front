    ...
    const handleSaveNotice = async () => {
        try {
            ...
            if (isNoticeEditing) { // 수정
                await axios.patch(`/teachers/notification-boards/${currentNotice.noticeId}`, formData, {
                    headers: {
    ...
    const handleDeleteNotices = async () => {
        try {
            if (selectedNotices.length > 0) { // 공지사항 삭제
                ...
                await axios.delete(`/teachers/notification-boards/${currentNotice.noticeId}`, {
                    headers: {
                    },
                });
            }
            ...
            {notices.map((notice) => (
                <div key={notice.id} className="notice-item">
                    <div className="notice-summary">
                        ...
                        />
                        <span className={`notice-type ${notice.isEmergency ? 'urgent' : 'normal'}`}>
              {notice.isEmergency ? '긴급' : '공지'}
            </span>
                    ...
                    {expandedNoticeId === notice.id && (
                        <div className="notice-content">
                            <button className="notice-edit-button" onClick={() => handleOpenNoticeModal({
                                noticeId: notice.id,
                                noticeType: notice.isEmergency ? '긴급' : '공지',
