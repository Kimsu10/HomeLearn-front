...
const TeacherLecture = ({ baseUrl }) => {
  ...
  const { data: mainLectures } = useAxiosGet(
    `/teachers/subjects/${mainSubjectId}`,
    ""
  );

  // 과목 게시판 최신 4개
  const { data: recentSubjectBoards } = useAxiosGet(
    `/teachers/subjects/${mainSubjectId}/boards-recent?limit=4`,
    []
  );

  // 질문 게시판 최신 4개
  const { data: recentQuestions } = useAxiosGet(
    `/teachers/subjects/${mainSubjectId}/questions-recent?limit=4`,
    []
  );

  // 강의 영상 (페이징)
  const { data: lecturePage } = useAxiosGet(
    `/teachers/subjects/${mainSubjectId}/lectures?page=0`,
    {}
  );
        ...
        <div className="teacher_lecture_subject_board_container">
          <div className="teacher_board_title_box">
            ...
            <span
              ...
              onClick={() =>
                navigate(`/teachers/${mainLectures?.subjectId}/board/list`)
              }
            >
              더보기 ⟩
            </span>
          ...
          <div className="teacher_inquiry_list_container">
            {inquiryBoards.map((el, idx) => (
              <div
                ...
                key={idx}
                onClick={() =>
                  navigate(`/teachers/questionBoards/${el.questionId}`)
                }
              >
                <div className="teacher_inquiry_title_box">
                  <div className="teacher_inquiry_type">질문</div>
                  <h4 className="teacher_inquiry_list_title">
                    {truncateText(el.title, 20)}
                  </h4>
